...
import closeForm from "../../utils/closeFeedbackForm";
...
export default function FeedbackForm() {
    ...
    return (
        <div
            id="feedback-form-bg-blur"
            className="h-full w-full fixed left-1/2 top-1/2 bg-black bg-opacity-75 backdrop-blur-sm invisible -translate-x-1/2 -translate-y-1/2">
            <form
                id="feedback-form"
                ...
                onSubmit={handleSubmit}>
                    ...
                    <textarea
                        ...
                        name="message"
                        placeholder="Write a detailed statement telling me how to improve this site and also what you like so far..."
                        maxLength={1000}
                        value={formData.message}
                        onChange={handleChange}
                        required>
                    </textarea>
                    <div className="absolute left-1/2 bottom-8 flex flex-row gap-5 -translate-x-1/2 smooth-change">
                        <button
                            className="px-4 py-2 border-2 border-midRed rounded-md bg-none text-midRed hover:border-darkRed hover:text-darkRed transition-colors duration-150"
                            onClick={closeForm}>
                                Cancel
                            </button>
                        <button
                            className="px-4 py-2 rounded-md bg-midGreen hover:bg-darkGreen text-white transition-transform duration-300"
                            type="submit">
                            Submit Report
                        </button>
                    </div>
